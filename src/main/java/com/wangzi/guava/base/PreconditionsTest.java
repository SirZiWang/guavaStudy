...
public class PreconditionsTest extends TestCase{
	 ...
	 */
	public void test(){
		doSomething("tom", 18, "Preconditions test!");
	}
	
	public void doSomething(String name, int age, String desc){
		Preconditions.checkNotNull(name, "name %s may not be null", "test");
		Preconditions.checkArgument(age>=18 && age<=99, "age must in rang(18,99)");
		Preconditions.checkArgument(desc !=null && desc.length() < 30, "desc too long, max length is ", 30);
